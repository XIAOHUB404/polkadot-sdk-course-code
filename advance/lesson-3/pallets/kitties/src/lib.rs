...
pub mod pallet {
    ...
    #[pallet::storage]
    pub type Kitties<T> = StorageMap<_, _, u64, Kitty>;

    #[pallet::storage]
    pub type KittyOwner<T: Config> = StorageMap<_, _, u64, T::AccountId>;

    // bid price for each kitty,
    #[pallet::storage]
    pub type KittiesBid<T: Config> = StorageMap<_, _, u64, Vec<(T::AccountId, BalanceOf<T>)>>;
    #[pallet::storage]
